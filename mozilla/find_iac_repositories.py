...
# Define the range of months to check for commits (e.g., 3 months ago)
def get_commits_in_range(repo, months=3):
    commits = get_commit_messages(repo)
    if not commits:
        return []

    now = datetime.now()
    start_date = now - timedelta(days=months * 30)  # Roughly approximates the last 3 months

    # Filter commits within the last `months` months
    commits_in_range = [c for c in commits if isinstance(c, dict) and start_date <= datetime.strptime(c['commit']['author']['date'], '%Y-%m-%dT%H:%M:%SZ') <= now]

    return commits_in_range

# Adjust Criteria-3 to check for at least two commits per month over the last 3-6 months
# Filter repositories based on the presence of Puppet files.
def filter_repositories(repos):
    ...
    for repo in repos:
        ...
        if isinstance(repo, dict) and not repo.get('private', True) and repo.get('size', 0) > 0:
            ...
            puppet_files = [f for f in files if any(f['path'].endswith(ext) for ext in puppet_extensions)]
            other_files = [f for f in files if not any(f['path'].endswith(ext) for ext in puppet_extensions)]
            print(f"Repo: {repo['full_name']}, Puppet files: {len(puppet_files)}, Other files: {len(other_files)}")
            if len(other_files) != 0 and len(puppet_files) / len(other_files) >= 0.11:
                commits = get_commit_messages(repo)
